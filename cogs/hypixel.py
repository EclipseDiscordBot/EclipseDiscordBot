...
class Hypixel(commands.Cog):
    ...
    async def _stats(self, ctx: commands.Context, uname: str = None, sb_profile_number: int = 1):
        ...
        async with ch.typing():
            ...
            bedwars_e.add_field(
                name="Stars",
                value=all_stats['achievements']['bedwars_level'])
            bedwars_e.add_field(
                name="Wins",
                value=all_stats['achievements']['bedwars_wins'])
            bedwars_e.add_field(
                name="Total games played",
                value=all_stats['stats']['Bedwars']['games_played_bedwars'])
            bedwars_e.add_field(
                name="Win Streak",
                value=all_stats['stats']['Bedwars']['winstreak'])
            bedwars_e.add_field(name="Beds Broken",
                                value=all_stats['achievements']['bedwars_beds'])
            bedwars_e.add_field(
                name="Deaths",
                value=all_stats['stats']['Bedwars']['deaths_bedwars'])
            bedwars_e.add_field(name="Coins",
                                value=all_stats['stats']['Bedwars']['coins'])
            bedwars_e.add_field(
                name="Kills",
                value=all_stats['stats']['Bedwars']['kills_bedwars'])
            bedwars_e.add_field(
                name="Total Items purchased",
                value=all_stats['stats']['Bedwars']['items_purchased_bedwars'])
            # Skywars
            skywars_e = discord.Embed(title=f'Skywars', colour=random_color)
            skywars_e.add_field(name="Souls",
                                value=all_stats['stats']['SkyWars']['souls'])
            skywars_e.add_field(
                name="Soul Well Level",
                value=all_stats['stats']['SkyWars']['soul_well'])
            skywars_e.add_field(
                name="Total games played",
                value=all_stats['stats']['SkyWars']['games_played_skywars'])
            skywars_e.add_field(name="Coins",
                                value=all_stats['stats']['SkyWars']['coins'])
            skywars_e.add_field(name="Deaths",
                                value=all_stats['stats']['SkyWars']['deaths'])
            skywars_e.add_field(
                name="Win Streak",
                value=all_stats['stats']['SkyWars']['win_streak'])
            skywars_e.add_field(name="Kills",
                                value=all_stats['stats']['SkyWars']['kills'])
            skywars_e.add_field(
                name="Kill Streak",
                value=all_stats['stats']['SkyWars']['killstreak'])
            skywars_e.add_field(name="Wins",
                                value=all_stats['stats']['SkyWars']['wins'])
            # Duels
            duels_e = discord.Embed(title=f'Duels', colour=random_color)
            duels_e.add_field(name="Coins",
                              value=all_stats['stats']['Duels']['coins'])
            duels_e.add_field(
                name="Total Games Played",
                value=all_stats['stats']['Duels']['games_played_duels'])
            duels_e.add_field(
                name="Blocks placed",
                value=all_stats['stats']['Duels']['blocks_placed'])
            duels_e.add_field(
                name="Total damage dealt",
                value=all_stats['stats']['Duels']['damage_dealt'])
            duels_e.add_field(name="Wins",
                              value=all_stats['stats']['Duels']['wins'])
            duels_e.add_field(name="Deaths",
                              value=all_stats['stats']['Duels']['deaths'])
            duels_e.add_field(
                name="Total Bridge Goals",
                value=all_stats['stats']['Duels']['goals'])
            # Skyblock
            skyblock_e = discord.Embed(title=f'Skyblock', colour=random_color)
            try:
                skyblock_e.add_field(
                    name="Profile name", value=list(
                        all_stats['stats']['SkyBlock']['profiles'].values())[
                        sb_profile_number - 1]['cute_name'])
            except IndexError:
                await ctx.reply("Invalid Profile Index, Try again :/")
                return
            prof_id = list(all_stats['stats']['SkyBlock']['profiles'].values())[
                sb_profile_number - 1]['profile_id']
            skyblock_inst = json.loads((await self.hypixel.profile(prof_id)).json())
            skyblock_info_root = skyblock_inst['members'][uuid]
            skyblock_e.add_field(
                name="Purse",
                value=f"{round(skyblock_info_root['coin_purse']):,}")
            skyblock_e.add_field(
                name="Bank",
                value=f"{((round(skyblock_inst['raw']['banking']['balance'])) if 'banking' in skyblock_inst['raw'] else 0):,}")
            skyblock_e.add_field(
                name="Highest Damage Dealt",
                value=f"{((round(skyblock_info_root['stats']['highest_critical_damage'])) if 'highest_critical_damage' in skyblock_info_root['stats'] else 0):,}")
            ...
            _paginator = SpecialPaginator(
                message, [bedwars_e, skywars_e, duels_e, skyblock_e])
