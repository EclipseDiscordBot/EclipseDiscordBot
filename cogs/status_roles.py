...
class status_roles(commands.Cog):
  ...
  async def sr(self,ctx):
    ...
    if key == "placeholder":
      key = "Not setup yet"
    embed = discord.Embed(title = "Status Roles", description = f"Status roles gives a role to a person if they have a certain keyword in their custom activity.\n **Here is {ctx.guild.name}'s configuration**", color = self.bot.color)
    ...
    if role is None:
      ...
      txt = role.mention
    embed = discord.Embed(title = "Status Roles has been updated!", description = f"Status roles gives a role to a person if they have a certain keyword in their custom activity.\n **Here is {ctx.guild.name}'s configuration**", color = self.bot.color)
    embed.add_field(name = "Role", value = txt)
